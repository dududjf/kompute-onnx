...
{{
    ...
    int v = floatBitsToInt(in_tensor[gi]);
    v = ~v;
    out_tensor[gi] = intBitsToFloat(v);
}}''')
    ...
    def run(self, *inputs):
        input_tensors = []
        original_dtype = inputs[0].dtype
        for inp in inputs:
            if np.issubdtype(inp.dtype, np.integer):
                numpy_in = inp.astype(np.int32).view(np.float32).reshape(-1)
            else:
