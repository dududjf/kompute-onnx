...
class CompressOp:
    def __init__(self, manager: kp.Manager, axis=None):
        ...
        self.axis = axis
        self.shader_no_axis = compile_source(f"""
#version 450

layout(local_size_x = {LOCAL_X_1D}) in;
layout(std430, set=0, binding=0) readonly  buffer InBuf     {{ float in_tensor[];     }};
layout(std430, set=0, binding=1) readonly  buffer CondBuf   {{ uint  cond_tensor[];   }};
layout(std430, set=0, binding=2) readonly  buffer PrefixBuf {{ uint  prefix_tensor[]; }};
...
""")
        ...
        self.shader_axis = compile_source(f"""
#version 450
...
void main() 
{{
    uint group_count = params[0], dimension = params[1];
    uint block_size = params[2], out_block_size = params[3];

    uint gx = gl_GlobalInvocationID.x;
    uint gy = gl_GlobalInvocationID.y;
    if (gx >= group_count || gy >= dimension) return;
...
""")
    ...
    def __repr__(self):
        ...
        return f"CompressOp({device_name})"
    ...
    def fuse(self, input_tensors: list[tuple[kp.Tensor, list[int]]], updated_algorithms: list[kp.Algorithm],
        ...
        if axis is not None:
            ...
            params = np.array([group_count, dimension, block_size, out_block_size], dtype=np.uint32)
            ...
            group_y = (dimension + LOCAL_Y_2D - 1) // LOCAL_Y_2D
            workgroup = (group_x, group_y, 1)

            updated_algorithms.append(
                self.manager.algorithm(
                    [tensor_in, tensor_condition, tensor_prefix, tensor_out, param_in],
                    self.shader_axis,
                    workgroup
                )
            )
            ...
            active_len = min(condition.size, int(np.prod(shape_in)))
            ...
            params = np.array([active_len], dtype=np.uint32)
            param_in = self.manager.tensor_t(params, kp.TensorTypes.device)
            self.manager.sequence().record(kp.OpTensorSyncDevice([param_in])).eval()

            group_x = (active_len + LOCAL_X_1D - 1) // LOCAL_X_1D
