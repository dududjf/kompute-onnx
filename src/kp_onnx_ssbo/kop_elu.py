...
layout(std430, set=0, binding=2) readonly  buffer AlphaBuf {{ float alpha_tensor[]; }};
...
""")
    ...
    def fuse(self, input_tensors: list[tuple[kp.Tensor, list[int]]], updated_algorithms: list[kp.Algorithm],
        ...
        tensor_alpha = self.manager.tensor(np.array([self.alpha], dtype=np.float32))
        updated_tensors.append(tensor_out)

        params = np.array([size], dtype=np.uint32)
        param_in = self.manager.tensor_t(params, kp.TensorTypes.device)
        self.manager.sequence().record(kp.OpTensorSyncDevice([tensor_alpha, param_in])).eval()

        group_x = (size + LOCAL_X_1D - 1) // LOCAL_X_1D
        workgroup = (group_x, 1, 1)

        updated_algorithms.append(self.manager.algorithm([tensor_in, tensor_out, tensor_alpha, param_in],
