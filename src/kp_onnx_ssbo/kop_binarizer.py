...
class BinarizerOp:
    def __init__(self, manager: kp.Manager, threshold: float = 0.0):
...
layout (std430, set = 0, binding = 2) readonly  buffer UIParam {{ 
    uint bound_x; 
    float threshold; 
}};
...
{{
    ...
    def __repr__(self):
        ...
        return f"BinarizerOp({device_name}, threshold={self.threshold})"
    ...
    def fuse(self, input_tensors: list[tuple[kp.Tensor, list[int]]], updated_algorithms: list[kp.Algorithm],
        ...
        param_size = np.array([size], dtype=np.uint32)
        param_thresh = np.array([self.threshold], dtype=np.float32).view(np.uint32)
        param_data = np.concatenate((param_size, param_thresh))
        param_in = self.manager.tensor_t(param_data, kp.TensorTypes.device)
