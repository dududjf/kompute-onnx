...
1. 利用Storage Buffer Object传递算子参数，这样不需要将int32或uint32类型的参数强制转成float32后通过spec_consts或push_consts传递，避免参数精度损失，比如uint32可以表达的最大正整数为2^32-1，但转成float32后只能表达的最大正整数降到2^24；
2. 全部算子采用shader代码实现，可以利用大于1的local_size_x、local_size_y和local_size_z设置多GPU线程并行操作。
