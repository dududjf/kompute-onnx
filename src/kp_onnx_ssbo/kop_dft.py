...
class DFTOp:
    ...
    def __init__(self, manager: kp.Manager, inverse=0, onesided=0):
        self.inverse = inverse
        self.onesided = onesided
        self.manager = manager
        self.dft_shader = compile_source(f"""
#version 450
...
void main() {{
    ...
    uint input_n = params[1];
    uint dft_length = params[2];
    uint output_n = params[3];
    uint post_size = params[4];
    float norm = uintBitsToFloat(params[5]);
    float angle_base = uintBitsToFloat(params[6]);
    uint in_idx_step = params[7];
    uint is_real_input = params[8];

    if(pre_idx >= pre_size || k >= output_n || post_idx >= post_size) return;
    ...
    uint in_idx = (is_real_input != 0u) ? in_idx_base : in_idx_base * 2;
    uint in_step = (is_real_input != 0u) ? post_size : in_idx_step;
    ...
    for (uint n = 0; n < loop_n; ++n) {{
        ...
        float x_imag = (is_real_input != 0u) ? 0.0 : in_buf[in_idx + 1];
...
""")
    ...
    def fuse(self, input_tensors: list[tuple[kp.Tensor, list[int]]], updated_algorithms: list[kp.Algorithm],
        ...
        params = np.array([
            ...
            actual_output_n,
            post_size,
            np.float32(norm).view(np.uint32),
            np.float32(angle_base).view(np.uint32),
            in_idx_step,
            1 if is_real_input else 0
        ], dtype=np.uint32)
        ...
        group_y = (actual_output_n + LOCAL_Y_3D - 1) // LOCAL_Y_3D
