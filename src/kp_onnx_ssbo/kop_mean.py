import kp
import numpy as np
from .shader_utils import compile_source, broadcast_to, LOCAL_X_1D, LOCAL_X_3D, LOCAL_Y_3D, LOCAL_Z_3D


class MeanOp:
    def __init__(self, manager: kp.Manager):
        self.manager = manager
        self.add_shader = compile_source(f'''
#version 450
...
void main()
{{
    uint size_x_1 = params[0], size_y_1 = params[1], size_z_1 = params[2];
    uint size_x_2 = params[3], size_y_2 = params[4], size_z_2 = params[5];
    ...
    uint stride_y_1 = size_z_1, stride_x_1 = size_y_1 * stride_y_1;
    uint stride_y_2 = size_z_2, stride_x_2 = size_y_2 * stride_y_2;
    ...
    out_tensor[gx * stride_x + gy * stride_y + gz] = in_tensor_1[p_1] + in_tensor_2[p_2];
        ...
        self.scale_shader = compile_source(f'''
#version 450
...
layout (std430, set = 0, binding = 2) readonly  buffer FParams {{ 
    float scale;
    uint bound_x;
}};
void main() {{
    ...
    out_tensor[idx] = in_tensor[idx] * scale;
}}
''')
    ...
    def fuse(self, input_tensors: list[tuple[kp.Tensor, list[int]]], updated_algorithms: list[kp.Algorithm],
        ...
        def _binary_add_plan(x, y):
            ...
            updated_algorithms.append(self.manager.algorithm(
                [new_in_1, new_in_2, output_tensor, param_in],
                self.add_shader,
        ...
        params_scale = np.array([scale], dtype=np.float32).view(np.uint32)
        params_scale = np.concatenate([params_scale, np.array([output_size], dtype=np.uint32)])
        param_scale_in = self.manager.tensor_t(params_scale, kp.TensorTypes.device)
        self.manager.sequence().record(kp.OpTensorSyncDevice([param_scale_in])).eval()
        updated_tensors.append(param_scale_in)

        workgroup_scale = ((output_size + LOCAL_X_1D - 1) // LOCAL_X_1D, 1, 1)
        algo_scale = self.manager.algorithm(
            [sum_tensor, tensor_out, param_scale_in],
            self.scale_shader,
            workgroup_scale
        )
