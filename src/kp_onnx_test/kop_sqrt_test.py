...
sqrt_op = SqrtOp(mgr, ['input'], ['output'])
...
#整数
int_in = np.random.randint(1, 1000, 1024 * 1024 * 16, dtype=np.int32)

start_time = time.time()
int_numpy_out = np.sqrt(int_in)
print("Numpy:", time.time() - start_time, "seconds")

start_time = time.time()
int_kp_out = sqrt_op.run(int_in)[0]
print(f"{sqrt_op}:", time.time() - start_time, "seconds")
...
start_time = time.time()
bool_numpy_out = np.sqrt(bool_in.astype(np.float32))
print("Numpy:", time.time() - start_time, "seconds")

start_time = time.time()
bool_kp_out = sqrt_op.run(bool_in.astype(np.float32))[0]
print(f"{sqrt_op}:", time.time() - start_time, "seconds")
