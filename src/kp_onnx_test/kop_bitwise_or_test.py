...
import numpy as np
import time
from kp_onnx_ssbo.kop_bitwise_or import BitwiseOrOp


device_id = 0
mgr = Manager(device_id)
print(mgr.get_device_properties())
or_op = BitwiseOrOp(mgr)
...
numpy_out = np.bitwise_or(numpy_in_1, numpy_in_2)
print("Numpy:", numpy_out.shape, time.time() - start_time, "seconds")

start_time = time.time()
kp_out = or_op.run(numpy_in_1, numpy_in_2)[0]
print(f"{or_op}:", kp_out.shape, time.time() - start_time, "seconds")
