...
atan_op = AtanOp(mgr, ['input'], ['output'])
...
start_time = time.time()
int_numpy_out = np.arctan(int_in)
print("Numpy:", time.time() - start_time, "seconds")

start_time = time.time()
int_kp_out = atan_op.run(int_in)[0]
print(f"{atan_op}:", time.time() - start_time, "seconds")
...
start_time = time.time()
bool_numpy_out = np.arctan(bool_in.astype(np.float32))
print("Numpy:", time.time() - start_time, "seconds")

start_time = time.time()
bool_kp_out = atan_op.run(bool_in.astype(np.float32))[0]
print(f"{atan_op}:", time.time() - start_time, "seconds")
