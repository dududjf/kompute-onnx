...
def pool(padded, x_shape, kernel, strides, out_shape, pooling_type, pads_required=None, pads=None, dilations=None, count_include_pad=0, p=2):
        ...
        *[
        ...
        if pooling_type == "AVG":
            ...
            def lp_pool(x: np.array, p: int = p) -> float:
                if len(x) == 0:
                    return 0.0
                valid_x = x[np.where(~np.isnan(x))]
                if len(valid_x) == 0:
                    return 0.0
                sum_p = np.sum(np.abs(valid_x) ** p)
                if count_include_pad == 1:
                    sum_p += np.sum(np.zeros(len(x) - len(valid_x)) ** p)
                return sum_p ** (1.0 / p)
            f = lp_pool
        else:
