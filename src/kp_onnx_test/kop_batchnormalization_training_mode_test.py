...
import numpy as np
import time
from kp_onnx_ssbo.kop_batchnormalization_training_mode import BatchNormalizationTrainingModeOp
...
batch_norm_training_op = BatchNormalizationTrainingModeOp(mgr)
...
def np_batch_norm_training(x, scale, bias, mean, var, epsilon=None, momentum=None, training_mode=None):
    if training_mode == 0:
...
np_out = np_batch_norm_training(x, scale, bias, mean, var, epsilon=1e-05, momentum=0.9, training_mode=1)
print("NumPy:", time.time() - start_time, "seconds")

start_time = time.time()
batch_norm_training_op.epsilon = 1e-05
batch_norm_training_op.momentum = 0.9
batch_norm_training_op.training_mode = 1
kp_out = batch_norm_training_op.run(x, scale, bias, mean, var)
print(f"{batch_norm_training_op}: ", time.time() - start_time, "seconds")
...
# -------- Case 2: data: 5D --------
print("Case 2: data: 5D")
x = np.random.random((16, 8, 8, 32, 32)).astype(np.float32)
scale = np.random.random((8,)).astype(np.float32)
bias = np.random.random((8,)).astype(np.float32)
mean = np.random.random((8,)).astype(np.float32)
var = np.random.random((8,)).astype(np.float32)
...
start_time = time.time()
np_out = np_batch_norm_training(x, scale, bias, mean, var, epsilon=1e-06, momentum=0.5, training_mode=1)
print("NumPy:", time.time() - start_time, "seconds")

start_time = time.time()
batch_norm_training_op.epsilon = 1e-06
batch_norm_training_op.momentum = 0.5
batch_norm_training_op.training_mode = 1
kp_out = batch_norm_training_op.run(x, scale, bias, mean, var)
print(f"{batch_norm_training_op}: ", time.time() - start_time, "seconds")
