...
import numpy as np
import time
from kp_onnx_ssbo.kop_bitwise_and import BitwiseAndOp


device_id = 0
mgr = Manager(device_id)
print(mgr.get_device_properties())
and_op = BitwiseAndOp(mgr)
...
numpy_out = np.bitwise_and(numpy_in_1, numpy_in_2)
print("Numpy:", numpy_out.shape, time.time() - start_time, "seconds")

start_time = time.time()
kp_out = and_op.run(numpy_in_1, numpy_in_2)[0]
print(f"{and_op}:", kp_out.shape, time.time() - start_time, "seconds")
