...
# Case 4: 更多维度 N C S1 S2 S3 S4 S5 （5 个空间维）
print("Case 4: N C S1 S2 S3 S4 S5")
numpy_in = np.random.uniform(-1, 1, (2, 3, 4, 3, 5, 2, 6)).astype(np.float32)

start_time = time.time()
axes = tuple(range(2, numpy_in.ndim))  # (2,3,4,5,6)
numpy_out = onnx_gap(numpy_in)
print("NumPy:", numpy_out.shape, time.time() - start_time, "seconds")
