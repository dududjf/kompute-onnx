...
void main()
{
    ...
    out_tensor[gx * stride_x + gy * stride_y + gz] = min(in_tensor_1[p_1], in_tensor_2[p_2]);
}''')

    def __repr__(self):
        device_name = self.manager.get_device_properties()['device_name']
        return f"MinOp({device_name})"
    ...
    def run(self, *inputs):
        ...
        def _binary_min_run(x, y):
            ...
            for i in range(len(new_shape_1)):
                if new_shape_1[i] == 1:
                    ...
                    assert new_shape_1[i] == new_shape_2[i], f"MinOp requires input {i} of the same shape"
                    output_shape.append(new_shape_1[i])
        ...
        for next_tensor in inputs[1:]:
            outputs = _binary_min_run(outputs, next_tensor)[0]
        return [outputs]
    ...
    def fuse(self, input_tensors: list[tuple[kp.Tensor, list[int]]], updated_algorithms: list[kp.Algorithm],
        ...
        def _fuse_two(f2_input: list[tuple[kp.Tensor, list[int]]]  # or tuple[…]
                      ) -> tuple[kp.Tensor, list[int]]:
            assert len(f2_input) == 2, "_fuse_two expects exactly two inputs"
            input_1 = f2_input[0][0]
            input_2 = f2_input[1][0]
            shape_1 = f2_input[0][1]
            shape_2 = f2_input[1][1]

            if len(shape_1) < len(shape_2):
                new_shape_1, new_shape_2 = [1] * (len(shape_2) - len(shape_1)) + shape_1, shape_2
            elif len(shape_2) < len(shape_1):
                new_shape_1, new_shape_2 = shape_1, [1] * (len(shape_1) - len(shape_2)) + shape_2
            else:
                new_shape_1, new_shape_2 = shape_1, shape_2
            output_shape = []
            for i in range(len(new_shape_1)):
                if new_shape_1[i] == 1:
                    output_shape.append(new_shape_2[i])
            ...
            updated_algorithms.append(
                self.manager.algorithm(
                    [new_in_1, new_in_2, tensor_out],
                    self.compiled_shader,  # shader 内部是：out = min(in1, in2)
                    workgroup,
                    [size_x_1, size_y_1, size_z_1, size_x_2, size_y_2, size_z_2],
                    []
                )
        ...
        for i in range(1, len(input_tensors)):
            cur, output_shape = _fuse_two([cur, input_tensors[i]])
