...
class MinOp:
    def __init__(self, manager: kp.Manager):
        ...
        self.compiled_shader = compile_source(r'''
#version 450
...
void main(){
    ...
    def __repr__(self):
        return f"MinOp({self.manager.get_device_properties()['device_name']})"
    ...
    def fuse(self, input_tensors: list[tuple[kp.Tensor, list[int]]], updated_algorithms: list[kp.Algorithm],
        ...
        def _binary_min_plan(x, y):
            ...
            for j in range(len(new_shape_1)):
                ...
                if a == 1:
                    ...
                    assert a == b, f"MinOp requires input {j} of the same shape"
                    output_shape.append(a)
        ...
        for i in range(1, len(input_tensors)):
            cur = _binary_min_plan(cur, input_tensors[i])

        tensor_out, shape_out = cur
