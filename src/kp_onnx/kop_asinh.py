...
from pyshader.stdlib import asinh


@python2shader
def compute_shader_asinh(index=("input", "GlobalInvocationId", ivec2),
                         in_data=("buffer", 0, Array(f32)),
                         out_data=("buffer", 1, Array(f32))):
    i = index.x
    out_data[i] = asinh(in_data[i])


_asinh_code = compute_shader_asinh.to_spirv()
...
class AsinhOp:
    ...
    def run(self, *inputs):
        ...
        algo = self.manager.algorithm([tensor_in, tensor_out], _asinh_code)
    ...
    def fuse(self, input_tensors: list[tuple[kp.Tensor, list[int]]], updated_algorithms: list[kp.Algorithm],
        ...
        updated_algorithms.append(self.manager.algorithm([tensor_in, tensor_out], _asinh_code))
