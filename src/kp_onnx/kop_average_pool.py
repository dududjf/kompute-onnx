...
void main() {
    ...
    uint in_idx_base = pre_idx * input_dim * post_size + post_idx;
    uint in_idx_step = post_size * dilation;
    uint in_idx = in_idx_base + start_pos * post_size;
    ...
    float denom = (cip == 1) ? kernel_size_f : max(valid_count, 1);
...
""")
    ...
    def fuse(self, input_tensors: list[tuple[kp.Tensor, list[int]]], updated_algorithms: list[kp.Algorithm],
        ...
        for dim in range(spatial_dims):
            if kernel_shape[dim] == 1 and strides_spatial[dim] == 1 and pads_final[dim] == 0 and pads_final[spatial_dims + dim] == 0:
                # 如果该维度不需要池化，跳过
                continue
