...
void main()
{
    ...
    float v1 = in_tensor_1[p_1];
    float v2 = in_tensor_2[p_2];
    out_tensor[gx * stride_x + gy * stride_y + gz] = (v1 > v2) ? v1 : v2;
}''')
    ...
    def run(self, *inputs):
        assert len(inputs) == 2, "MaxOp requires 2 inputs"
        input_1 = inputs[0]
        input_2 = inputs[1]
    ...
    def fuse(self, input_tensors: list[tuple[kp.Tensor, list[int]]], updated_algorithms: list[kp.Algorithm],
        ...
        assert len(input_tensors) == 2, "MaxOp requires 2 inputs"
