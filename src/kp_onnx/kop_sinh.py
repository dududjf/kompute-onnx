...
from pyshader.stdlib import sinh


@python2shader
def compute_shader_sinh(index=("input", "GlobalInvocationId", ivec2),
                        in_data=("buffer", 0, Array(f32)),
                        out_data=("buffer", 1, Array(f32))):
    i = index.x
    out_data[i] = sinh(in_data[i])


_sinh_code = compute_shader_sinh.to_spirv()
...
class SinhOp:
    ...
    def run(self, *inputs):
        ...
        algo = self.manager.algorithm([tensor_in, tensor_out], _sinh_code)
