...
class ReshapeOp:
    ...
    def run(self, *inputs):
        ...
        data = np.array(inputs[0], dtype=np.float32)
        target_shape = np.array(inputs[1], dtype=np.int64).tolist()
        allowzero = int(inputs[2]) if len(inputs) >= 3 else self.allowzero

        tensor_data = self.manager.tensor(data.reshape(-1))
        input_tensors = [
            (tensor_data, list(data.shape)),
            (tensor_data, target_shape),
        ]

        if len(inputs) > 2:
            tensor_allowzero = self.manager.tensor(np.array([allowzero], dtype=np.int32))
            input_tensors.append((tensor_allowzero, [allowzero]))
    ...
    def fuse(self, input_tensors: list[tuple[kp.Tensor, list[int]]], updated_algorithms: list[kp.Algorithm],
        ...
        input_tensor, input_shape_list = input_tensors[0]
        _, target_shape_list = input_tensors[1]

        if len(input_tensors) > 2:
            allowzero = input_tensors[2][1][0]
        else:
        ...
        assert np.prod(new_shape) == total, f"Reshape total elements mismatch: input={total}, new_shape={new_shape}"
