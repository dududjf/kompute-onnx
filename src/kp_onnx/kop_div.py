...
void main()
{
    ...
    def run(self, *inputs):
        assert len(inputs) == 2, "DivOp requires 2 inputs"
    ...
    def fuse(self, input_tensors: list[tuple[kp.Tensor, list[int]]], updated_algorithms: list[kp.Algorithm],
        ...
        assert len(input_tensors) == 2, "DivOp requires 2 inputs"
        ...
        for i in range(len(new_shape_1)):
            if new_shape_1[i] == 1:
                ...
                assert new_shape_1[i] == new_shape_2[i], f"DivOp requires input {i} of the same shape"
                output_shape.append(new_shape_1[i])
