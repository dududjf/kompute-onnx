...
class BitShiftOp:
    def __init__(self, manager: kp.Manager, direction: str = "RIGHT"):
        self.manager = manager

        if direction not in ("LEFT", "RIGHT"):
            raise ValueError(f"Unexpected value for direction ({direction!r}).")
        self.direction = direction

        self.compiled_shader_left = compile_source(self._shader_left_src)
        self.compiled_shader_right = compile_source(self._shader_right_src)

    _shader_left_src = '''#version 450
layout(local_size_x = 1, local_size_y = 1, local_size_z = 1) in;
layout(set=0, binding=0) buffer buf_in_tensor { float in_data[]; };
layout(set=0, binding=1) buffer buf_shift_tensor { float shift_data[]; };
layout(set=0, binding=2) buffer buf_out_tensor { float out_data[]; };
...
void main()
{
    ...
    int a_i = int(in_data[p_1]);
    int s_i = int(shift_data[p_2]);
    out_data[gx * stride_x + gy * stride_y + gz] = float(a_i << s_i);
}'''

    _shader_right_src = '''#version 450
layout(local_size_x = 1, local_size_y = 1, local_size_z = 1) in;
layout(set=0, binding=0) buffer buf_in_tensor { float in_data[]; };
layout(set=0, binding=1) buffer buf_shift_tensor { float shift_data[]; };
layout(set=0, binding=2) buffer buf_out_tensor { float out_data[]; };
...
void main()
{
    ...
    def __repr__(self):
        ...
        return f"BitShiftOp({device_name})"
    ...
    def run(self, *inputs):
        assert len(inputs) == 2, "BitShiftOp requires 2 inputs"
    ...
    def fuse(self, input_tensors: list[tuple[kp.Tensor, list[int]]], updated_algorithms: list[kp.Algorithm],
        ...
        assert len(input_tensors) == 2, "BitShiftOp requires 2 inputs"
        ...
        for i in range(len(new_shape_1)):
            if new_shape_1[i] == 1:
                ...
                assert new_shape_1[i] == new_shape_2[i], \
                    "BitShiftOp requires each dimension to be one or equal to the corresponding dimension"
                output_shape.append(new_shape_1[i])
        ...
        shader = self.compiled_shader_left if self.direction == "LEFT" else self.compiled_shader_right
