...
class WhereOp:
    def __init__(self, manager: kp.Manager):
        ...
        self.compiled_shader = compile_source('''#version 450
...
void main()
{
    ...
    def run(self, *inputs):
        assert len(inputs) == 3, "WhereOp requires 3 inputs (condition, x, y)"
    ...
    def fuse(self, input_tensors: list[tuple[kp.Tensor, list[int]]], updated_algorithms: list[kp.Algorithm],
        ...
        # Pad shapes with leading 1s to match the maximum number of dimensions
        ...
        for i in range(max_dims):
            # For each dimension, take the maximum size among all inputs
            max_size = max(new_shape_c[i], new_shape_x[i], new_shape_y[i])
            output_shape.append(max_size)

        # Process condition tensor broadcasting
        ...
        # Set workgroup size based on the maximum dimensions
